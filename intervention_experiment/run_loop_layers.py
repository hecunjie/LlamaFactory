...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Loop Last K Layers intervention on GSM8K.")
    parser.add_argument("--model_name_or_path", type=str, required=True)
    parser.add_argument("--output_path", type=str, required=True)
    parser.add_argument("--threshold", type=float, required=True)
    parser.add_argument("--loop_k", type=int, required=True)
...
def main():
    ...
    run_experiment(exp_args, method="loop_layers")
