...
def run_sft(
    ...
    if (
        ...
        or getattr(finetuning_args, "use_rgha", False)
    ):
        ...
        if add_think_added > 0:
            logger.info(
                ...
                f"use_rgha={getattr(finetuning_args, 'use_rgha', False)})"
            )
    ...
    # RGHA: do NOT replace model.forward here. HF Trainer's main CE uses outputs.logits; a patch
    # that overwrites logits with refined logits would train the base LM for the wrong graph and
    # break plain inference (use_rgha=false). RGHA modules + auxiliary loss live in CustomSeq2SeqTrainer.
